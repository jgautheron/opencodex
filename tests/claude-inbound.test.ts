...
describe("tool/system order stabilization (devlog 260727b_routed_tool_sort)", () => {
  ...
  test("skills-list normalization preserves a following reminder block", () => {
    const skills =
      "<system-reminder>\nUser-invocable skills:\n\n- zeta: z\n- alpha: a\n</system-reminder>";
    const following = "<system-reminder>\nKeep this block unchanged.\n</system-reminder>";
    const body = anthropicToResponsesBody({
      model: "m", max_tokens: 1, messages, system: `${skills}\n${following}`,
    }) as Record<string, any>;
    expect(body.instructions).toContain(following);
    expect(body.instructions.indexOf("- alpha: a")).toBeLessThan(body.instructions.indexOf("- zeta: z"));
  });
